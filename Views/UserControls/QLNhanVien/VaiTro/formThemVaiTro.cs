    ...
    public partial class FormThemVaiTro : Form
    {
        ...
        private void RenderQuyenCheckboxes()
        {
            ...
            foreach (var module in modules)
            {
                ...
                // Col 1: Checkbox Thêm (cùng row với module name) - BỎ CHECKED MẶC ĐỊNH
                var cbThem = new CheckBox
                {
                    Tag = themQuyen?.IdQuyen ?? 0,
                    Checked = false, // Luôn false ban đầu, chỉ checked khi user click
                    AutoSize = false,
                    Dock = DockStyle.Fill,
                ...
                // Col 2: Checkbox Sửa - BỎ CHECKED MẶC ĐỊNH
                ...
                // Col 3: Checkbox Xóa - BỎ CHECKED MẶC ĐỊNH
                ...
                // Col 4: Checkbox Chi tiết - BỎ CHECKED MẶC ĐỊNH
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                // Tạo và thêm vai trò mới
                var vaiTro = new Entities.VaiTro { TenVaiTro = tenVaiTro };
                _vaiTroService.AddVaiTro(vaiTro); // Thêm vai trò và save, IdVaiTro được set tự động

                // Thu thập các quyền được chọn từ checkboxes (chỉ nếu Tag > 0 và Checked = true)
                var selectedQuyenIds = _quyenCheckboxes
                    .Where(cb => cb.Checked && (int)cb.Tag > 0)
                    .Select(cb => (int)cb.Tag)
                    .ToList();

                // Gán quyền cho vai trò mới (lưu vào bảng vaitro_quyen)
                if (selectedQuyenIds.Any())
                {
                    _vaiTroService.AssignQuyenToVaiTro(vaiTro.IdVaiTro, selectedQuyenIds);
                }

                MessageBox.Show("Thêm vai trò và quyền thành công!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
