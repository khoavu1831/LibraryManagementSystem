...
namespace LMS.Views.UserControls.QLNhanVien.VaiTro
{
    public partial class FormSuaVaiTro : Form
    {
        ...
        private void BtnLuu_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var context = new LibraryDbContext())  // Single context for entire transaction
                {
                    // Load VaiTro with Quyens in this context (tracked)
                    var vaiTroToUpdate = context.VaiTros
                        .Include(v => v.Quyens)
                    ...
                    {
                        // Load Quyen in this context (will be tracked)
                        var quyen = context.Quyens.Find(idQuyen);
                        if (quyen != null)
                        {
